...
properties(i++)
{
  foreach_face() muv.x[] = fm.x[] * D * U0 / Reynolds;
}
...
init(t = 0)
{
  ...
  solid(
    cs,
    fs,
    intersection(intersection(0.5 - y, 0.5 + y), sqrt(sq(x) + sq(y)) - D / 2.));
...
* [Same example with
Gerris](http://gerris.dalembert.upmc.fr/gerris/examples/examples/cylinder.html)
*/
...
htg(i = 5; i += 5, i <= 10)
{
  output_hdf_htg({ p, f, spid }, { u }, "karman");
}