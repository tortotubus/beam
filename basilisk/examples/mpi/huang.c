...
defaults(i = 0)
{
  ...
  ib_beam_t beam1 = ib_beam_new_theta(b_length, b_EI, b_mu, b_nodes, b_r, b_theta);
  ib_mesh_model(&IBMESH(0), beam1);

  for (int ni = 0; ni < b_nodes; ni++) {
    IBMESH(0).nodes[ni].weight = b_mu * b_ds; 
    IBMESH(0).nodes[ni].gravity.x = 1; 
  }
}
...
vtk(t += DT_SAVE; t <= T_END)
{
  scalar omega[];
  vorticity(u, omega);
  output_hdf_htg({ omega, p, stencils, f}, { u }, "ex3");
}
...
event adapt (i++) {
  ...
  adapt_wavelet ({cs,u,f,stencils}, (double[]){1e-2,3e-2,3e-2,3e-2,3e-2}, maxlevel, 4);
}
