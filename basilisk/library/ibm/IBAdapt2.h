...
void cell_tags (int target_level, bool overwrite = false) {
  ...
  char path[64];
  snprintf (path, sizeof (path), "_%d.txt", pid ());
  FILE* fp = fopen (path, overwrite ? "w" : "a");
  if (!fp)
    return;

  int local_too_fine = 0;
  int local_ibm_protected = 0;
  int local_boundary_parents = 0;
  int local_boundary_too_fine = 0;
  int local_boundary_ibm_protected = 0;
  int local_nonleaf = 0;
  int printed = 0;

  foreach_cell () {
    if (level != target_level || is_leaf (cell) || !is_local (cell))
      continue;
    ...
    if (is_boundary (cell)) {
      local_boundary_parents++;
      if (cell.flags & too_fine)
        local_boundary_too_fine++;
      if (just_fine_ibm_children)
        local_boundary_ibm_protected++;
    }
    ...
    if (printed < 16 &&
        ((cell.flags & too_fine) || just_fine_ibm_children || is_boundary (cell))) {
      fprintf (
        fp,
        "pid=%d target_level=%d x=%g y=%g z=%g boundary=%d too_fine=%d "
        "refined=%d too_coarse=%d just_fine=%d just_fine_ibm_children=%d\n",
        pid (), target_level, x, y, z, is_boundary (cell),
        !!(cell.flags & too_fine), !!(cell.flags & refined),
        !!(cell.flags & too_coarse), !!(cell.flags & just_fine),
        just_fine_ibm_children);
      printed++;
    }
  }

  fprintf (fp,
           "summary pid=%d target_level=%d local_nonleaf=%d "
           "local_too_fine=%d local_ibm_protected=%d "
           "local_boundary_parents=%d local_boundary_too_fine=%d "
           "local_boundary_ibm_protected=%d printed=%d\n",
           pid (), target_level, local_nonleaf, local_too_fine,
           local_ibm_protected, local_boundary_parents, local_boundary_too_fine,
           local_boundary_ibm_protected, printed);
  fclose (fp);
}
                          ...
                          scalar* list = all) // list of fields to update
{
  ...
  for (int l = depth (); l >= 0; l--) {
    // Clear coarsening requests on any parent whose children were marked as
    // IBM-critical. This includes MPI-boundary parents, which are not handled
    // by the local-only loop below but are still seen by mpi_boundary_coarsen().
    foreach_cell () {
