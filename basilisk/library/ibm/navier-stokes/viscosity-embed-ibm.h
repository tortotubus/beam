...
#define lambda                                                                 \
  ((coord) {0,                                                                 \
            dt * (mu.x[] + mu.x[1] + mu.y[] + mu.y[0, 1]) * sq (cs[]) /        \
              (fm.x[] + fm.x[1] + fm.y[] + fm.y[0, 1] + SEPS) /                \
              (cm[] + SEPS)})
...
static void relax_diffusion_ibm (scalar* a, scalar* b, int l, void* data) {
  ...
  foreach_level_or_leaf (l, nowarning) {
    double avgmu = 0.;
    foreach_dimension () avgmu += mu.x[] + mu.x[1];
    avgmu = dt * avgmu + SEPS;
    foreach_dimension () {
      double c = 0.;
      ...
      scalar s = u.x;
      double a = 0.;
      foreach_dimension () a += mu.x[1] * s[1] + mu.x[] * s[-1];
...
static double
residual_diffusion_ibm (scalar* a, scalar* b, scalar* resl, void* data) {
...
trace mgstats viscosity_ibm (vector u,
                             face vector mu,
                             scalar rho,
                             vector ibmf,
                             double dt,
  ...
  // foreach ()
  //   foreach_dimension () r.x[] = rho[] * u.x[];

  foreach () {
    foreach_dimension() {
      r.x[] = rho[] * u.x[];// + dt * rho[] * ibmf.x[];
    }
  }
  ...
  return mg_solve ((scalar*) {u},
                   (scalar*) {r},
                   residual_diffusion_ibm,
                   relax_diffusion_ibm,
                   &p,
                   nrelax,
                   res,
