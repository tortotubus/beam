...
static double residual_viscosity_ibm (scalar * a, scalar * b, scalar * resl, 
				  void * data)
{
  struct Viscosity * p = (struct Viscosity *) data;
  (const) face vector mu = p->mu;
...
trace
mgstats viscosity_ibm (vector u, face vector mu, scalar rho, vector ibmf, double dt,
		   int nrelax = 4, scalar * res = NULL)
{
  ...
  return mg_solve ((scalar *){u}, (scalar *){r},
		   residual_viscosity_ibm, relax_viscosity, &p, nrelax, res);
}
