...
#include "library/io/vtk/vtkType.h"

/**
 * @brief This struct and the associated members collect descriptive information
 * from basilisk to describe the tree as a HyperTreeGrid.
 */
typedef struct {
  size_t max_vertices; /**< Number of cells in the tree level with the greatest
                          number of cells */
  int64_t depth_per_tree; /**< Maximum depth of our tree */

  Bit_t* descriptors;      /**< Breadth-first search of our tree. See @ref
                              vtk_hdf_get_descriptors */
  ...
  int64_t* number_of_cells_per_tree_depth; /**< Number of the cells (per-level)
                                              in the tree */
  int64_t
    number_of_trees; /**< Number of trees; in basilisk this is always 1. */
...
/**
 * @brief Count the number of cells on each level of the (binary/quad/oct)tree.
 * Also compute the maximum
 *
 * @memberof vtkHDFHyperTreeGridData
 */
void vtk_hdf_get_number_of_vertices_per_depth (
  vtkHDFHyperTreeGridData* vtk_hdf_hypertreegrid_data) {
  ...
  memset (vtk_hdf_hypertreegrid_data->number_of_cells_per_tree_depth,
          0,
...
/**
 * @brief Traverse the (binary-/quad-/oct-)tree using breadth-first search and
 * record if the node is refined or not.
 *
 * This function uses the custom @ref foreach_cell_BFS() iterator macro to
 * perform breadth first search. If the nth visited node is not a leaf (i.e. it
 * is refined), then we set the corresponding bit in our byte array to 1 (true).
 * Otherwise we set it to 0 (false).
 *
 * @memberof vtkHDFHyperTreeGridData
 */
void vtk_hdf_get_descriptors (
  vtkHDFHyperTreeGridData* vtk_hdf_hypertreegrid_data) {
  ...
  // We decide that the total number of descriptors (bits) we need is the number
  // of cells in all but the deepest level: Since this is the deepest level then
  // it follows that any nodes there must be leaves, and so any leaves there are
  // implied by the 1 from their parents
  size_t n_descriptors = 1;
  if (max_depth > 1)
    n_descriptors =
      total_vertices -
...
vtkHDFHyperTreeGridData* vtk_hdf_hypertreegrid_data_init (void) {
  ...
  if (!grid) {
    fprintf (stderr,
             "Grid has not yet been initialized: Please initialize the "
             "grid first.\n");
    exit (1);
  }
