...
event interface_force_richardson (i++, last) {
  ...
  for (int k = 0; k < ib_richardson_iters; k++) {
    ...
    // Update the working field for Richardson convergence and accumulate the
    // force density to be applied through the acceleration/projection path.
    foreach () {
      foreach_dimension () {
  ...
  face vector af = a;
  foreach_face () af.x[] = 0.5 * alpha.x[] * (ibmf.x[] + ibmf.x[-1]);
}
