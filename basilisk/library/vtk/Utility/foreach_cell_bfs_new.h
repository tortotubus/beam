macro2 foreach_cell_root_BFS(Point root) {
  {
    ...
    queue_t *queue_current =
        (queue_t *)malloc(current_size_max * sizeof(queue_t));
    ...
    while (current_count > 0) {
      for (int i = 0; i < current_count; i++) {
        ...
        if (point.level < grid->depth && !is_leaf(cell)) {
          ...
          queue_next[next_count++] =
              (queue_t){point.level + 1, _RIGHT, _BOTTOM};
        ...
        if (point.level < grid->depth && !is_leaf(cell)) {
          queue_next[next_count++] =
              (queue_t){point.level + 1, _LEFT, _BOTTOM, _BACK};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _RIGHT, _BOTTOM, _BACK};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _LEFT, _TOP, _BACK};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _RIGHT, _TOP, _BACK};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _LEFT, _BOTTOM, _FRONT};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _RIGHT, _BOTTOM, _FRONT};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _LEFT, _TOP, _FRONT};
          queue_next[next_count++] =
              (queue_t){point.level + 1, _RIGHT, _TOP, _FRONT};
        }
#endif
