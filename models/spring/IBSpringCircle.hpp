 ...
 *
 * @note This class inherits from IBVelocityCoupled and
 *       implements a velocity-coupled immersed boundary method compatible
 *       with fluid solvers.
 */
class IBSpringCircle : public IBVelocityCoupled
{
protected:
...
public:
  ...
  IBSpringCircle(int NumberOfPoints,
                 ...
                 real_t center_x,
                 real_t center_y)
    : IBVelocityCoupled(NumberOfPoints)
    , K(K)
    , radius_x(radius)
    , radius_y(radius)
    , dtheta((2 * M_1_PI) / NumberOfPoints)
  {
    std::vector<IBMesh::IBVertex> circle_points =
  ...
  IBSpringCircle(int NumberOfPoints,
    ...
    , dtheta((2 * M_1_PI) / NumberOfPoints)
  {
    std::vector<IBMesh::IBVertex> ellipse_point =
...
protected:
   ...
   *
   * @see IBVelocityCoupled::ComputeMidpointForces()
   */
  void ComputeMidpointForces() override
  {
    std::vector<IBMesh::IBVertex>& forces = mesh_next.GetForces();
...
private:
   ...
   */
  static std::vector<IBMesh::IBVertex> circle(int N,
                                                       real_t radius,
                                                       real_t center_x,
                                                       real_t center_y)
  {
    std::vector<IBMesh::IBVertex> points(N);
   ...
   */
  static std::vector<IBMesh::IBVertex> ellipse(int N,
                                                        real_t radius_x,
                                                        real_t radius_y,
                                                        real_t center_x,
                                                        real_t center_y)
