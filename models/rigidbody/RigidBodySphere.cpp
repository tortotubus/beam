...
namespace {
constexpr real_t kPi = static_cast<real_t>(3.141592653589793238462643383279502884L);
...
RigidBodySphere::RigidBodySphere(real_t radius, size_t point_count, real_t density)
  : radius_(radius)
  , point_count_(point_count)
  , density_(density)
{
  ELFF_VERIFY(radius_ > 0., "RigidBodySphere: radius must be positive.\n");
  ELFF_VERIFY(point_count_ > 0, "RigidBodySphere: point_count must be positive.\n");
...
void
RigidBodySphere::define_reference_configuration(std::vector<Vec3>& points_ref,
                                                std::vector<real_t>& ds,
                                                Vec3& cog_ref,
                                                std::vector<Vec3>& normals_ref) const
{
  points_ref.resize(point_count_);
  ds.assign(point_count_, 4. * kPi * square(radius_) / static_cast<real_t>(point_count_));
  ...
  for (size_t i = 0; i < point_count_; ++i) {
    const real_t y = 1. -
                     2. * (static_cast<real_t>(i) + static_cast<real_t>(0.5)) /
                       static_cast<real_t>(point_count_);
    const real_t r_xy = std::sqrt(std::max(static_cast<real_t>(0.), 1. - y * y));
...
RigidBodySphere::define_mass_properties(real_t& mass, Mat3& I_body) const
{
  ...
  I_body = {{{inertia, 0., 0.}, {0., inertia, 0.}, {0., 0., inertia}}};
}
