...
void
EulerBeamInextensibleADDM::collect_boundary_dofs(
...
void
EulerBeamInextensibleADDM::solve()
{
  solve({ 0., 0., 0. });
}

void
EulerBeamInextensibleADDM::solve(std::array<real_t, 3> load)
{
...
void
EulerBeamInextensibleADDM::solve(real_t dt, std::array<real_t, 3> load)
{
...
void
EulerBeamInextensibleADDM::solve(real_t dt,
...
void
EulerBeamInextensibleADDM::apply_initial_condition()
{
...
void
EulerBeamInextensibleADDM::apply_initial_condition(EulerBeamMesh& bmesh)
{
...
const VectorXd&
EulerBeamInextensibleADDM::get_lambda_x() const
{
  return lambda_x;
}

const VectorXd&
EulerBeamInextensibleADDM::get_lambda_y() const
{
  return lambda_y;
}

const VectorXd&
EulerBeamInextensibleADDM::get_lambda_z() const
{
  return lambda_z;
}

const MatrixXd&
EulerBeamInextensibleADDM::get_A() const
{
...
void
EulerBeamInextensibleADDM::apply_initial_condition_xy()
{
...
void
EulerBeamInextensibleADDM::initialize_quadrature_cache()
{
  const real_t h = mesh.get_ds();
...
void
EulerBeamInextensibleADDM::compute_slopes_collocation()
{
  xp.setZero();
  yp.setZero();
  zp.setZero();
  ...
  for (size_t ei = 0; ei < elements; ei++) {
    ...
    xp[nodes + ei] = midpoint_dH[0] * x[edofs[0]] +
                     midpoint_dH[1] * x[edofs[1]] +
                     midpoint_dH[2] * x[edofs[2]] +
                     midpoint_dH[3] * x[edofs[3]];
    yp[nodes + ei] = midpoint_dH[0] * y[edofs[0]] +
                     midpoint_dH[1] * y[edofs[1]] +
                     midpoint_dH[2] * y[edofs[2]] +
                     midpoint_dH[3] * y[edofs[3]];
    zp[nodes + ei] = midpoint_dH[0] * z[edofs[0]] +
                     midpoint_dH[1] * z[edofs[1]] +
                     midpoint_dH[2] * z[edofs[2]] +
                     midpoint_dH[3] * z[edofs[3]];
  }
}

void
EulerBeamInextensibleADDM::apply_initial_condition_pq()
{
  const size_t nodes = this->mesh.get_nodes();
...
void
EulerBeamInextensibleADDM::apply_boundary_condition_lambda()
{
  const size_t nodes = mesh.get_nodes();
  for (size_t bi = 0; bi < 2; ++bi) {
    const EulerBeamBCType bctype = boundary_conditions.type[bi];
...
void
EulerBeamInextensibleADDM::update_pq()
{
  const size_t nodes = mesh.get_nodes();
  compute_slopes_collocation();
...
void
EulerBeamInextensibleADDM::apply_boundary_condition_A()
{
  A = A_unconstrained;
...
void
EulerBeamInextensibleADDM::assemble_A()
{
  ...
  Matrix<real_t, 4, 4> constraint_element_matrix =
    Matrix<real_t, 4, 4>::Zero();
  ...
  for (size_t e = 0; e < this->elements; ++e) {
    ...
    for (size_t a = 0; a < 4; ++a) {
      for (size_t b = 0; b < 4; ++b) {
        bending_triplets.emplace_back(
          edofs[a], edofs[b], bending_element_matrix(a, b));
        constraint_triplets.emplace_back(
          edofs[a], edofs[b], constraint_element_matrix(a, b));
      }
    }
  }
  ...
  K_bending.setFromTriplets(
    bending_triplets.begin(), bending_triplets.end(), std::plus<real_t>());
  K_constraint.setFromTriplets(
    constraint_triplets.begin(), constraint_triplets.end(), std::plus<real_t>());
...
void
EulerBeamInextensibleADDM::decompose_A()
{
  llt.compute(A);
...
void
EulerBeamInextensibleADDM::clear_rhs()
{
  f_x.setZero();
  f_y.setZero();
  f_z.setZero();
...
void
EulerBeamInextensibleADDM::assemble_constraint_rhs()
{
  const size_t nodes = this->mesh.get_nodes();
  ...
  for (size_t e = 0; e < elements; ++e) {
    ...
    cx_elem << lambda_x[li] + r_penalty * p[li],
               lambda_x[mi] + r_penalty * p[mi],
               lambda_x[ri] + r_penalty * p[ri];
    cy_elem << lambda_y[li] + r_penalty * q[li],
               lambda_y[mi] + r_penalty * q[mi],
               lambda_y[ri] + r_penalty * q[ri];
    cz_elem << lambda_z[li] + r_penalty * r[li],
               lambda_z[mi] + r_penalty * r[mi],
               lambda_z[ri] + r_penalty * r[ri];
...
void
EulerBeamInextensibleADDM::add_uniform_load_rhs(std::array<real_t, 3> load)
{
  const real_t h = this->mesh.get_ds();
...
void
EulerBeamInextensibleADDM::add_nodal_load_rhs(
...
void
EulerBeamInextensibleADDM::add_point_boundary_loads()
{
...
void
EulerBeamInextensibleADDM::assemble_f(std::array<real_t, 3> load)
{
  clear_rhs();
...
void
EulerBeamInextensibleADDM::apply_boundary_condition_f()
{
  std::vector<size_t> idx;
  std::vector<real_t> xvals, yvals, zvals;
  collect_boundary_dofs(idx, xvals, yvals, zvals);
...
void
EulerBeamInextensibleADDM::update_xy(std::array<real_t, 3> load)
{
  assemble_f(load);
...
void
EulerBeamInextensibleADDM::update_multipliers()
{
  const size_t nodes = mesh.get_nodes();
  compute_slopes_collocation();
...
bool
EulerBeamInextensibleADDM::is_converged(const VectorXd& x_old_iter,
...
{
  return compute_relative_state_update(
           x_old_iter, x_new_iter, y_old_iter, y_new_iter, z_old_iter, z_new_iter) <
         tol_outer;
}

void
EulerBeamInextensibleADDM::solve_newmark(real_t dt,
...
void
EulerBeamInextensibleADDM::solve_newmark(
  real_t dt,
  std::vector<std::array<real_t, 3>> load,
  real_t beta,
  real_t gamma)
...
void
EulerBeamInextensibleADDM::assemble_mass_matrix()
{
  mass_diag.setZero();
...
void
EulerBeamInextensibleADDM::prepare_system_newmark(real_t dt)
{
...
void
EulerBeamInextensibleADDM::assemble_system_newmark_rhs(
...
void
EulerBeamInextensibleADDM::add_averaged_uniform_load_rhs(
  std::array<real_t, 3> load)
{
...
void
EulerBeamInextensibleADDM::add_averaged_nodal_load_rhs(
  const std::vector<std::array<real_t, 3>>& load)
{
  ELFF_ASSERT(load.size() == mesh.get_nodes(),
              "Size of load vector must equal number of nodes.");
...
void
EulerBeamInextensibleADDM::apply_midpoint_bending_rhs()
{
  const real_t coeff = -0.5 * EI;

  for (size_t e = 0; e < elements; ++e) {
    const std::array<size_t, 4> edofs = {
      2 * (e + 0) + 0, 2 * (e + 0) + 1, 2 * (e + 1) + 0, 2 * (e + 1) + 1
    };
...
void
EulerBeamInextensibleADDM::apply_dynamic_state_boundary_conditions()
{
  const size_t nodes = mesh.get_nodes();
...
real_t
EulerBeamInextensibleADDM::compute_relative_state_update(
...
real_t
EulerBeamInextensibleADDM::compute_max_pq_error() const
{
...
real_t
EulerBeamInextensibleADDM::compute_max_state_update(
  const VectorXd& x_old_iter,
  const VectorXd& x_new_iter,
  const VectorXd& y_old_iter,
  const VectorXd& y_new_iter,
...
void
EulerBeamInextensibleADDM::update_average_acceleration_state_component(
