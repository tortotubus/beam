...
public:
  EulerBeamDynamicInextensibleADDM(real_t length,
    ...
    , previous_times(2)
    , xt(MatrixXd::Zero(dof, previous_times))
    , yt(MatrixXd::Zero(dof, previous_times))
    , zt(MatrixXd::Zero(dof, previous_times))
  {
    assemble_M();
  };
  ...
  void assemble_A_dyn_BE() { A = M + (dt * dt) * A; }
  ...
  void update_pq() override { EulerBeamStaticInextensibleADDM::update_pq(); }
