...
Vector::Vector(const Vector &v)
{
   ...
   if (s > 0)
   {
      BEAM_ASSERT(!v.data.Empty(), "invalid source vector");
...
Vector &Vector::operator=(real_t value)
{
   ...
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] = value; });
   return *this;
}
...
Vector &Vector::operator*=(real_t c)
{
   ...
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] *= c; });
   return *this;
}

Vector &Vector::operator*=(const Vector &v)
{
   BEAM_ASSERT(size == v.size, "incompatible Vectors!");

   const bool use_dev = UseDevice() || v.UseDevice();
   const int N = size;
   auto y = ReadWrite(use_dev);
   auto x = v.Read(use_dev);
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] *= x[i]; });
   return *this;
}
...
Vector &Vector::operator/=(real_t c)
{
   ...
   auto y = ReadWrite(use_dev);
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] *= m; });
   return *this;
}
...
{
   BEAM_ASSERT(size == Va.size, "incompatible Vectors!");
   ...
   if (a != 0.0)
   {
      ...
      mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] += a * x[i]; });
   }
   return *this;
}
...
{
   BEAM_ASSERT(v.Size() + offset <= size, "invalid sub-vector");
   ...
   mfem::forall_switch(use_dev, vs, [=] BEAM_HOST_DEVICE (int i) { p[i] = vp[i]; });
}
...
void Vector::Neg()
{
   ...
   const int N = size;
   auto y = ReadWrite(use_dev);
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] = -y[i]; });
}
...
void Vector::Reciprocal()
{
   ...
   const int N = size;
   auto y = ReadWrite(use_dev);
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] = 1.0/y[i]; });
}

void add(const Vector &v1, const Vector &v2, Vector &v)
{
   BEAM_ASSERT(v.size == v1.size && v.size == v2.size,
               "incompatible Vectors!");

#if !defined(BEAM_USE_LEGACY_OPENMP)
   ...
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { y[i] = x1[i] + x2[i]; });
#else
...
void add(const Vector &v1, real_t alpha, const Vector &v2, Vector &v)
{
   ...
   else
   {
      ...
      mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i)
      {
...
{
   BEAM_ASSERT(x.size == y.size && x.size == z.size,
               "incompatible Vectors!");
...
void Vector::cross3D(const Vector &vin, Vector &vout) const
{
   ...
   vin.HostRead();
   vout.HostWrite();
   BEAM_VERIFY(size == 3, "Only 3D vectors supported in cross.");
   BEAM_VERIFY(vin.Size() == 3, "Only 3D vectors supported in cross.");
   vout.SetSize(3);
...
{
   BEAM_ASSERT(size == lo.size && size == hi.size,
               "incompatible Vectors!");
...
{
   BEAM_ASSERT(dofs.Size() <= elemvect.Size(),
...
{
   BEAM_ASSERT(dofs.Size() <= elemvect.Size(), "Size mismatch: "
...
void Vector::SetSubVectorComplement(const Array<int> &dofs, const real_t val)
{
   ...
   auto d_dofs = dofs.Read(use_dev);
   mfem::forall_switch(use_dev, n, [=] BEAM_HOST_DEVICE (int i) { d_dofs_vals[i] = d_data[d_dofs[i]]; });
   mfem::forall_switch(use_dev, N, [=] BEAM_HOST_DEVICE (int i) { d_data[i] = val; });
   mfem::forall_switch(use_dev, n, [=] BEAM_HOST_DEVICE (int i) { d_data[d_dofs[i]] = d_dofs_vals[i]; });
}
...
real_t Vector::Norml2() const
{
   ...
   reduce(
      size, res,
      [=] BEAM_HOST_DEVICE(int i, value_type &r)
   {
...
real_t Vector::Normlinf() const
{
   ...
   reduce(
      size, res,
   [=] BEAM_HOST_DEVICE(int i, real_t &r) { r = fmax(r, fabs(m_data[i])); },
...
real_t Vector::Norml1() const
{
   ...
   reduce(
      size, res,
   [=] BEAM_HOST_DEVICE(int i, real_t &r) { r += fabs(m_data[i]); },
...
{
   BEAM_ASSERT(p > 0.0, "Vector::Normlp");
...
real_t Vector::operator*(const Vector &v) const
{
   ...
   if (use_dev)
   {
      ...
      {
#define BEAM_USE_OPENMP_DETERMINISTIC_DOT
#ifdef BEAM_USE_OPENMP_DETERMINISTIC_DOT
         ...
         return prod;
#endif // BEAM_USE_OPENMP_DETERMINISTIC_DOT
      }
#endif // BEAM_USE_OPENMP
   }
   ...
   reduce(
      size, res,
   [=] BEAM_HOST_DEVICE(int i, real_t &r) { r += m_data[i] * v_data[i]; },
...
real_t Vector::Min() const
{
   ...
   reduce(
      size, res,
   [=] BEAM_HOST_DEVICE(int i, real_t &r) { r = fmin(r, m_data[i]); },
...
real_t Vector::Max() const
{
   ...
   reduce(
      size, res,
   [=] BEAM_HOST_DEVICE(int i, real_t &r) { r = fmax(r, m_data[i]); },
...
real_t Vector::Sum() const
{
   ...
   real_t res = 0;
   reduce(
   size, res, [=] BEAM_HOST_DEVICE(int i, real_t &r) { r += m_data[i]; },
   SumReducer<real_t> {}, UseDevice(), vector_workspace());
   return res;
}
