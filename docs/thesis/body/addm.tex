...
be recovered by taking \(\vec f = -\rho_b g \vec e_y\), up to the sign convention
...
Introducing the beam tension \(T(s)\), equivalently the Lagrange multiplier for
...
\begin{equation*}
    \mat M \ddot{\vec q} + \mat C \dot{\vec q} + f^{\rm int}(\vec q) = f^{\rm ext}
\end{equation*}
where \(\mat M\) is a mass matrix, and \(\mat C\) is a damping matrix. Newmark-\(\beta\) scheme provides for the update
\begin{align*}
    \vec q_{n+1}        & = \vec q_n + \Delta t \dot{\vec q}_n + \frac12 \Delta t^2 \ddot {\vec q}_\beta \\
    \dot {\vec q}_{n+1} & = \dot{\vec q}_n + \Delta t \dot{\vec q}_\gamma
\end{align*}
where the acceleration vector \(\ddot{\vec q}_\beta\) and the velocity vector \(\dot{\vec q}_\gamma\) are defined as
\begin{align*}
    \ddot{\vec q}_\beta   & = (1-2\beta) \ddot{\vec q}_n + 2\beta\ddot{\vec q}_{n+1}  \\
    \dot{\vec q}_{\gamma} & = (1- \gamma)\ddot{\vec q}_n + \gamma \ddot{\vec q}_{n+1}
\end{align*}
Substituted into the updates we have
\begin{align*}
    \vec q_{n+1}        & = \vec q_n + \Delta t \dot{\vec q}_n + \frac12 \Delta t^2  \left[(1-2\beta) \ddot{\vec q}_n + 2\beta \ddot{\vec q}_{n+1}\right] \\
    \dot {\vec q}_{n+1} & = \dot{\vec q}_n + \Delta t \left[(1- \gamma)\ddot{\vec q}_n + \gamma \ddot{\vec q}_{n+1}\right]
\end{align*}
which gives stencils for \(\ddot{\vec q}_{n+1}\) and \(\dot{\vec q}_{n+1}\):
\begin{align*}
    \ddot{\vec q}_{n+1} & = \frac{1}{\beta \Delta t^2}(\vec q_{n+1} - \vec{q}_n - \Delta t \dot{\vec q}_n) - \frac{1-2\beta}{2\beta}\ddot{\vec q}_n \\
    \dot{\vec q}_{n+1}  & = \dot{\vec q}_n + \Delta t[(1-\gamma)\ddot{\vec q}_n + \gamma \ddot{\vec q}_{n+1}]
\end{align*}
In our time dependent E-B problem we have
\begin{equation*}
    \rho_b \ddot{\vec r}(s) + \underbrace{EI \vec{r}^{(4)}(s) - (T(s) \vec r'(s))'}_{\vec f^{\rm int}} =  \underbrace{\vec f(s,t)}_{\vec f^{\rm ext}}
\end{equation*}
...
\subsection{Elliptic Integral Solution to the Static Problem}
...
\begin{figure}[H]
    ...
    \caption{Non-dimensional tip deflection \(y_{\rm tip}/L\) and tip retreat \(x_{\rm tip}/L\) with respect to non-dimensional tip load \(PL^2/B\), reproduced from \textcite{bisshopp_large_1945}}
...
\subsection{Pinned Manufactured Solution to the Dynamic Problem}
...
\begin{equation*}
    \rho_b \ddot{\vec r} + EI \vec r^{(4)} = \vec f
\end{equation*}
...
\begin{equation*}
    \vec f_{\rm linear}
    =
    \left[\rho_b\left(\frac1\phi - s^2\phi\right)+EI\phi^3\right]\vec e_r
    - \frac{\rho_b}{\phi}\vec e_x
    - \rho_b s\vec e_\theta.
\end{equation*}
...
\begin{equation*}
    \vec f_{\rm nonlinear}
    =
    \rho_b\ddot{\vec r}_{\rm ex}
    + EI\vec r_{\rm ex}^{(4)}
    + \pdv*{\left(T \pdv{\vec r_{\rm ex}}{s}\right)}{s}.
\end{equation*}
The multiplier contribution is
\begin{equation*}
    \pdv*{\left(T \pdv{\vec r_{\rm ex}}{s}\right)}{s}
    =
    \pdv{T}{s}\vec e_\theta - T\phi\vec e_r.
\end{equation*}
With the manufactured choice \(T=1\), this contribution is
...
\begin{equation*}
    \vec f_{\rm nonlinear}
    =
    \left[\rho_b\left(\frac1\phi - s^2\phi\right)+EI\phi^3-\phi\right]\vec e_r
    - \frac{\rho_b}{\phi}\vec e_x
    - \rho_b s\vec e_\theta.
\end{equation*}
...
decrease in error as the time step is refined.
...
choice \(T=1\). This test exercises both the Newmark discretization and the
nonlinear ADDM iteration used to enforce inextensibility.
